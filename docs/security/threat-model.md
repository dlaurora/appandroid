...
MVP Android app, offline-first and local-first. Future data may include customer details, contact details, addresses, quotes, amounts, technical report text, photos, exported files, and backups.
...
| Android backup captures client data unexpectedly | Local Room database | Auto Backup or device transfer includes persisted clients before an approved backup/import feature | Unexpected disclosure or uncontrolled restore behavior | Medium | Exclude database domain in backup and data extraction rules during Phase 2 | Mitigated for Phase 2 | Inspect backup XML and manifest |
...
| Sensitive data in logs | Customer and commercial data | Developer logs or release logs include user data | Data leakage | Medium | No client-field logging in Phase 2; future central logging wrapper must redact sensitive values | Open | Static search and release build review |
