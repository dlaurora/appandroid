...
MVP Android app, offline-first and local-first. Implemented data includes customer details, local catalog service/product records, quotes, line items, amounts, discounts, taxes, notes, and terms. Future data may include technical report text, photos, exported files, and backups.
...
| Android backup captures local app data unexpectedly | Local Room database | Auto Backup or device transfer includes persisted clients, catalog records, or quotes before an approved backup/import feature | Unexpected disclosure or uncontrolled restore behavior | Medium | Exclude database domain in backup and data extraction rules during Phase 4 | Mitigated for Phase 4 | Inspect backup XML and manifest |
| Unauthorized exported quote or report access | Future quotes, reports, PDFs | Another app or person accesses exported files after a future export/share phase | Exposure of personal and commercial data | Medium | Share only by explicit user action; use content URIs; avoid broad storage permissions | Not started | FileProvider and sharing tests when implemented |
...
| Quote mistaken for fiscal/legal document | Local quotes and future generated documents | User treats quote as invoice, contract, certificate, tax record, or legal document | Business or legal risk | Medium | Terms of use and UI copy avoid compliance claims; future document footer required when generation exists | Open | Content review before publication and before PDF/export |
